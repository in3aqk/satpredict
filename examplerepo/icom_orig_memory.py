...
class IcomUtilities:
    ...
    def __init__(self):
        ...
        self.radio_configurations = {
            ...
            'IC-7100-Home': ('IC-7100', ('general_hf', 'marine_hf', 'marine_vhf_wx', 'general_vhf_uhf', 'emergency_beacon', 'marine_vhf_long', 'family_radio_service', 'cb', 'vhf_repeaters_long', 'E')),
        }
        ...
        # Windows: COM1, COM2, etc
        self.mac_port = '/dev/tty.SLAB_USBtoUART2'
    ...
    def array_to_xhtml(self, radio_name, array):
        ...
        footer += self.wrap_tag('a', 'IcomProgrammer',
                                'href=\"http://arachnoid.com/IcomProgrammer\"')
        ...
        for tup in array:
            for datum in tup:
                ...
                if re.search('table:number-columns-repeated', datum):
                    ...
                    sn = re.sub(
                        '.*table:number-columns-repeated=\"(\d+)\".*', '\\1', datum)
                    n = int(sn)
                if re.search('/>', datum):
...
class IcomIO:
    ...
    def program_radio(self, array, radio_id, banksize, radio_tag, erase_unused=False):
        if(not self.init_serial()):
            sys.stderr.write(
                'Error: cannot open serial interface, aborting.\n')
            return
            ...
            if(not radio_model in self.icom_codes):
                ...
                for i, record in enumerate(master_array[1:]):
                    if(bank_size > 0):
                        ...
                        record[1] = '%02d' % (i + 1)
                if(commit):
                    self.program_radio(master_array, hex_code,
                                       bank_size, radio_model, erase_unused)
                master_array = self.remove_empty_fields_from_array(
                    master_array)
                self.encode_write_xsv_file(os.path.join(
                    self.rcsv_directory, tag + '.csv'), master_array, '","', '"')
                self.encode_write_xsv_file(os.path.join(
                    self.rtsv_directory, tag + '.tsv'), master_array, '\t', '')
                html_path = os.path.join(self.html_directory, tag + '.html')
                html_page = self.encode_write_xhtml_file(
                    tag, html_path, master_array)
                self.encode_write_pdf_file(tag, html_page, os.path.join(
                    self.pdf_directory, tag + '.pdf'))
    ...
    def create_xsv_files(self):
        ...
        for fn in files:
            if(re.match('(?i).*\\.ods$', fn)):
                ...
                self.encode_write_xsv_file(os.path.join(
                    self.fcsv_directory, fn + '.csv'), records, '","', '"')
                self.encode_write_xsv_file(os.path.join(
                    self.ftsv_directory, fn + '.tsv'), records, '\t', '')
